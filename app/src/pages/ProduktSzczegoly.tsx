...
const MOCK_REVIEWS = [
  {
    ...
    rating: 5,
    date: "2026-03-15",
    text: "Absolutnie wspaniały smak! Zamawiam regularnie i nigdy się nie zawiodłam. Aromat jest niesamowity, a smak pełny i bogaty.",
  },
  {
    author: "Tomasz M.",
    city: "Kraków",
    rating: 5,
    date: "2026-03-02",
    text: "Jeden z najlepszych produktów jakie próbowałem. Idealny do porannej rutyny — daje energię i przyjemność.",
  },
  {
    author: "Ania W.",
    city: "Gdańsk",
    rating: 4,
    date: "2026-02-18",
    text: "Bardzo dobry produkt. Delikatny aromat i przyjemna tekstura. Jedyny minus to trochę za wysoka cena.",
  },
  {
    author: "Michał R.",
    city: "Wrocław",
    rating: 5,
    date: "2026-02-05",
    text: "Polecam każdemu kto ceni sobie jakość. Świeży, aromatyczny i doskonale zapakowany.",
  },
  {
    author: "Kasia P.",
    city: "Poznań",
    rating: 4,
    date: "2026-01-22",
    text: "Smak wyśmienity, choć spodziewałam się nieco bardziej intensywnego aromatu. Mimo to — kupię ponownie!",
  },
];

const FLAVOR_PROFILES: Record<string, { label: string; value: number }[]> = {
  Herbata: [
    { label: "Świeżość", value: 85 },
    { label: "Goryczka", value: 30 },
    { label: "Słodycz", value: 60 },
    { label: "Umami", value: 45 },
    { label: "Kwiatowość", value: 70 },
    { label: "Intensywność", value: 55 },
  ],
  Kawa: [
    { label: "Kwasowość", value: 65 },
    { label: "Goryczka", value: 50 },
    { label: "Słodycz", value: 55 },
    { label: "Ciało", value: 75 },
    { label: "Aromat", value: 85 },
    { label: "Intensywność", value: 70 },
  ],
  Dodatki: [
    { label: "Słodycz", value: 80 },
    { label: "Aromat", value: 90 },
    { label: "Intensywność", value: 45 },
    { label: "Naturalność", value: 85 },
    { label: "Uniwersalność", value: 70 },
    { label: "Gęstość", value: 60 },
  ],
};
...
const BREWING_TIPS: Record<string, string[]> = {
  Herbata: [
    ...
    "Ilość: 2g na 200ml",
    "Najlepsza w porcelanie",
  ],
  Kawa: [
    "Temperatura wody: 92–96°C",
    "Mielenie: średnie",
    "Proporcja: 1:15 (kawa:woda)",
    "Najlepsza świeżo mielona",
  ],
  Dodatki: [
    "Dodaj 15–20ml na filiżankę",
    "Mieszaj energicznie",
    "Przechowuj w lodówce",
    "Najlepszy w ciągu 30 dni",
  ],
};
  ...
  <div style={{ display: "flex", gap: 2 }}>
    {[1, 2, 3, 4, 5].map((i) => (
      <svg
        ...
        height={size}
        viewBox="0 0 24 24"
        fill={i <= Math.round(rating) ? "#C4882A" : "none"}
        stroke="#C4882A"
        strokeWidth={1.5}
      >
...
export default function ProduktSzczegoly() {
  ...
  const [activeTab, setActiveTab] = useState<"opis" | "profil" | "opinie">(
    "opis",
  );
  const API =
  ...
  const mainImage =
    NAME_TO_IMAGE[(product.nazwa || "").toLowerCase()] ||
    FALLBACK_IMGS[product.id % FALLBACK_IMGS.length];
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#FAF7F2" }}>
      ...
      {/* ── Main content ── */}
      <div
        style={{ maxWidth: 1280, margin: "0 auto", padding: "48px 48px 80px" }}
      >
        <div
          style={{
            display: "grid",
        ...
        >
          ...
          <div>
            ...
            >
              {product.podkategoria || kategoria}
            </p>
            <h1
              style={{
                fontFamily: "'Cormorant Garamond',serif",
            ...
            >
              ...
              >
                {avgRating} · {MOCK_REVIEWS.length} opinii
              </span>
            </div>
            ...
            >
              {[product.kraj_pochodzenia, kategoria, product.podkategoria]
                .filter(Boolean)
                .map((t) => (
                  <span
                    key={t}
            ...
            >
              {[
                {
                  label: "Waga",
                  value: product.waga_g
                    ? `${product.waga_g}${product.jednostka === "ml" ? "ml" : "g"}`
                    : "-",
                },
                {
                  label: "Pochodzenie",
                  value: product.kraj_pochodzenia || "-",
                },
          ...
          >
            {(
              [
                ...
                ["profil", "Profil smakowy"],
                ["opinie", `Opinie (${MOCK_REVIEWS.length})`],
              ] as const
            ).map(([key, label]) => (
              <button
                key={key}
                ...
                style={{
                  ...
                  color: activeTab === key ? accent : "#9E7A5A",
                  borderBottom:
                    activeTab === key
                      ? `2px solid ${accent}`
                      : "2px solid transparent",
